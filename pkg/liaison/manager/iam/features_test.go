...
func TestFeaturePolicy_GrantRevokeAndRestart(t *testing.T) {
	...
	require.ErrorIs(t, s.SetUserFeaturePolicy(admin, []string{FeatureFilesUpload}), ErrInvalid)
	require.NoError(t, s.SetUserFeaturePolicy(admin, []string{FeatureFilesRead}))
	require.NoError(t, s.RequireFeature(user, FeatureFilesRead))
	require.ErrorIs(t, s.RequireFeature(user, FeatureFilesUpload), ErrForbidden)
	require.NoError(t, s.SetUserFeaturePolicy(admin, []string{FeatureFilesRead, FeatureFilesUpload}))
	require.NoError(t, s.RequireFeature(user, FeatureFilesUpload))
	require.NoError(t, s.SetUserFeaturePolicy(admin, []string{FeatureHomeAI, FeatureSettingsRead, FeatureAudit}))
	require.ErrorIs(t, s.RequireFeature(user, FeatureFilesRead), ErrForbidden)
	require.ErrorIs(t, s.RequireFeature(user, FeatureFilesUpload), ErrForbidden)
