...
func builtinRegistrations(router *Router) []tool.ToolRegistration {
	...
	return []tool.ToolRegistration{
		...
		registration(router, source, "data", "schema", OperationDataSchema,
			"Inspect data schema", "Inspect the attached database or S3 session. For S3, empty path returns browser_context (untrusted navigation hints) and visible buckets; path=[bucket,bucket_name,prefix,continuation_token] lists up to 200 prefixes and object metadata. Preserve exact keys. No file contents, writes or downloads are available to Agent. Listing may be partial: use next_token, never infer totals from one page. Empty path returns current_database and a tree of databases, schemas, tables, collections or keys. To inspect an object use path=[type,database,schema,name]: MySQL/MariaDB [table,db,\"\",table], PostgreSQL/SQL Server [table,db,schema,table], MongoDB [collection,db,\"\",collection], Redis [key,current_db_number,\"\",key]. Object details include columns/indexes/DDL for SQL, index-derived fields/indexes for MongoDB (not a complete document schema), and type/TTL/memory for Redis in its currently selected DB. Use names from the tree, do not invent them. Use an approved bounded query to sample documents or read key values.",
			[]tool.Protocol{tool.ProtocolS3, tool.ProtocolMemcached, tool.ProtocolElasticsearch, tool.ProtocolOpenSearch, tool.ProtocolMySQL, tool.ProtocolMariaDB, tool.ProtocolSQLServer, tool.ProtocolOracle, tool.ProtocolClickHouse, tool.ProtocolPostgreSQL, tool.ProtocolRedis, tool.ProtocolMongoDB}, []tool.Capability{"data.schema"}, tool.RiskReadOnly, tool.ApprovalNever,
		...
		registration(router, source, "data", "query", OperationDataQuery,
			"Query data", "Run one statement in the attached data session after approval. MySQL/MariaDB/PostgreSQL: SQL, prefer qualified table names and LIMIT. SQL Server: T-SQL with TOP (n), schema-qualified names, no LIMIT or GO separators. Redis: native command text (GET, SCAN, HGETALL, etc.), prefer SCAN over KEYS. MongoDB: a JSON database command, NOT mongosh JavaScript; e.g. {\"find\":\"items\",\"filter\":{},\"limit\":20}, {\"aggregate\":\"items\",\"pipeline\":[{\"$limit\":20}],\"cursor\":{}}. MongoDB executes in current_database from data.schema; do not claim to switch databases. Writes, updates, deletes and DDL all require approval. Explain failures using the returned error, never claim success for an error result.",
			[]tool.Protocol{tool.ProtocolMemcached, tool.ProtocolElasticsearch, tool.ProtocolOpenSearch, tool.ProtocolMySQL, tool.ProtocolMariaDB, tool.ProtocolSQLServer, tool.ProtocolOracle, tool.ProtocolClickHouse, tool.ProtocolPostgreSQL, tool.ProtocolRedis, tool.ProtocolMongoDB}, []tool.Capability{"data.query"}, tool.RiskHigh, tool.ApprovalByPolicy,
