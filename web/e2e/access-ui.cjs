...
(async()=>{
 ...
 try {for(const locale of ['zh-CN','en-US'])for(const theme of ['dark','light']){
  ...
  assert.equal(await selects.first().locator('option[value=aiapi]').textContent(),'OpenAI');
  ...
  for(const supported of ['tcp','http','ssh','webrdp','webvnc','aiapi'])assert(offered.includes(supported),`Missing supported access: ${supported}`);
  ...
  assert.equal(await llmRow.getByText('OpenAI',{exact:true}).count(),2);
